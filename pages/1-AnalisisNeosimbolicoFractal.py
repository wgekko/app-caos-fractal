    ...
    if date_col in df.columns and price_col in df.columns:
        ...
        with st.expander(":material/developer_mode_tv: Módulo de Fase (Spin): Análisis de Arquetipos", expanded=True):
            ...
            **Análisis de Spin Aplicado:**
            - El comportamiento del activo en la variable **'{price_col}'** corresponde a la esquina **{fase}**.
        ...
        if len(df) > 20:
            ...
            # --- NUEVO GRÁFICO DE CUADRANTES (ATRACTOR DEL CAOS) ---
            st.subheader(":material/map:  Mapa de Fases Angular: Atractor Dinero vs. Papel")
            
            # Armamos la trayectoria histórica de los últimos 30 periodos
            ...
            # 1. Dibujar Línea de Trayectoria Temporal
            ...
            # 2. Resaltar Estado Actual (Punto presente)
            fig_quad.add_trace(go.Scatter(
            ...
            # 3. Líneas divisoras del umbral del Caos (33 Grados)
            ...
            # Configuraciones estéticas y etiquetas de cuadrantes
        ...
        st.markdown("---")
        # --- MÓDULO 4: SÍNTESIS GLOBAL Y MEMORIA FRACTAL ---
        st.header("4. Veredicto del Sistema: Semáforo de Riesgo Global")
        
        # 1. Cálculo de Memoria Fractal (Hurst)
        hurst_val = calcular_hurst(df[price_col])
        
        # 2. Evaluación de Riesgos (Recopilación de Módulos Anteriores)
        nivel_riesgo = 0
        razones_riesgo = []
        
        # Evaluar Frontera 0.636
        ...
        #     if hurst_val < 0.45:
        #         nivel_riesgo += 1
        #         razones_riesgo.append(f"Antipersistencia (H={hurst_val:.2f}). Alta probabilidad de reversión violenta.")
        #     elif hurst_val > 0.65:
        #         estado_hurst = "Persistente (Tendencia Robusta y con Memoria)"
        #     else:
        ...
        # 3. Renderizado del Semáforo Ejecutivo
